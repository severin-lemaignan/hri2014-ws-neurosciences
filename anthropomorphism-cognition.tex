...
In the evolution of anthropomorphism over time, we propose to distinguish
three main phases, depicted in different shades in
Figure~\ref{fig:dynamics}.

\subsection*{Initialization} During this short phase (which lasts from a couple of seconds to a couple of hours), an increase of anthropomorphic effects is observed,
from the
\emph{initial capital of anthropomorphism} (ICA) to a peak of anthropomorphic manifestations that corresponds to the
...
phenomenon \cite{duffy_anthropomorphism_2003}. However, it has been shown, for instance,
...
We proposed that the ICA is built on three factors:
...
is speculative, and only indirectly supported by experimental evidence. More research
needs to be done to specifically test these
hypotheses.
...
factors that plays a key role in the dynamics of anthropomorphism.
In this preliminary report, we sketch a new
formal model of anthropomorphism that accounts for these factors and also
...
We discuss the cognitive correlates of anthropomorphism, and
...
research on long-term acceptance of robots in human environments. We
also hope that it may support further
discussions and reflections on how anthropomorphism impacts human-robot
interaction over the long run, and also foster research on the
affective bonds
